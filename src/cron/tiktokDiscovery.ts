...
import { crawlTiktokBuzz, type CrawlMode } from '../crawlers/tiktok.js'
import { upsertTiktokVideo, setCronLog, countTiktokVideos } from '../db/tiktokVideos.js'

let isRunning = false  // 동시 실행 방지

export async function runTiktokDiscovery(mode: CrawlMode = 'thorough'): Promise<{ ok: boolean; inserted: number; updated: number; total: number; durationSec: number; error?: string }> {
  if (isRunning) {
  ...
  isRunning = true
  const t0 = Date.now()
  console.log(`[TikTok cron] Discovery 시작 [${mode}]`)
  try {
    const { videos } = await crawlTiktokBuzz({ mode, topN: 30, commentsPerVideo: 20 })
    let inserted = 0
    let updated = 0
